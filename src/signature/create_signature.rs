...
use k256::{elliptic_curve::FieldBytes, Secp256k1};
use serde::Serialize;
...
pub(crate) fn sign_typed_data<S: SolStruct + Serialize>(
    payload: &S,
    wallet: &PrivateKeySigner,
) -> Result<PrimitiveSignature> {
    let typed_data = TypedData::from_struct(payload, Some(l1::domain()));
...
fn sign_hash(hash: B256, wallet: &PrivateKeySigner) -> Result<PrimitiveSignature> {
    let sig = wallet
        .sign_hash_sync(&hash)
...
mod tests {
    ...
    fn test_sign_l1_action() -> Result<()> {
        ...
        assert_eq!(
            serde_json::to_string(&sign_l1_action(&wallet, connection_id, true)?).unwrap(),
        ...
        assert_eq!(
            serde_json::to_string(&sign_l1_action(&wallet, connection_id, false)?).unwrap(),
    ...
    fn test_sign_usd_transfer_action() -> Result<()> {
        ...
        assert_eq!(
            serde_json::to_string(&sign_typed_data(&usd_send, &wallet)?).unwrap(),
            expected_sig
        );
        Ok(())
    }
